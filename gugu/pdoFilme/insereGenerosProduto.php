<?php
        ...
        try{
            foreach($generos as $g){
                $comandoSql = "INSERT INTO genero(id, descricao) VALUES({$g['id']}, '{$g['descricao']}')";
        ...
        try{
            foreach($filmes as $f){
                $comandoSql = "INSERT INTO filme(titulo, imdb, genero_id) VALUES('{$f['titulo']}', {$f['imdb']}, {$f['genero_id']})";
                $stmt = $conexao->prepare($comandoSql);
                $stmt->execute();
            }
            echo "Dados inseridos.";
